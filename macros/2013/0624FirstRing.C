...
//---------------------------
  ...
  Bool_t showmethering=0; //find and show me the reco ring
  ...
  for(int ecounter=0;ecounter<10;ecounter++){
    ...
    haReco->RunSuperFilter(theRecoEvent); //not working anymore
    ...
    //cout<< "Following display using vertex: ("<<theRecoEvent->GetVtxX()<<","<<theRecoEvent->GetVtxY()<<","<<theRecoEvent->GetVtxZ()<<endl;
  ...
  for(int ecounter=0;ecounter<10;ecounter++){
    ...
    haReco->RunSuperFilter(theRecoEvent);
