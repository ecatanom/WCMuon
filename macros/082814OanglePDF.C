{
  ...
  TH1D* hltrackdataangle = new TH1D("hltrackdataangle", ";Oangle [deg]",100,0,60);
 ...
 TH1D* hltrackmc = new TH1D("hltrackmc", "Ltrack [cm]",100,mindmu,maxdmu);
  ...
  for(int i=0; i<myNdigits; i++){
   ...
   //hltrackdataall->Fill(LMuon); //Ltrack only
    if(LMuon>=20 && LMuon <=350) {
      hltrackdataangle->Fill(Oangle);
    }
...
//****************************************************************
 ...
 if( distMuon > (softcutoff-earlycut) ) PLmu = Norm/(1.0+((distMuon-softcutoff+earlycut)*(distMuon-softcutoff+earlycut)/(latesigma*latesigma)));
  ...
  hltrackdataall->SetTitle("400 MeV 50 kton muon");
  ...
  hltrackdataall->Draw();
   ...
   hltrackdataangle->SetLineColor(kBlue);
   hltrackdataangle->SetTitle("400 MeV 50 kton muon, passed");
   hltrackdataangle->Draw("same");
  ...
  */
  ...
  TH1D * hNewOanglePDF0 = new TH1D("hOPDF","Opening Angle PDF; Oangle [deg];",100,0,60);

  for(int i=1;i<=100;i++){
    Double_t thebias= hltrackdataall->GetBinContent(i);
    Double_t thevalue =hltrackdataangle->GetBinContent(i);
    hNewOanglePDF0->SetBinContent(i,thevalue/thebias);
  }
